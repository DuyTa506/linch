...
| 18 | **`micro_compact` is copy-on-write** — provider-view and full-history reads are deep-copied snapshots; elision must build new `Message`/`ToolResultBlock` objects, never mutate in place, and its result is applied through `record_projection` (a logged projection), not by mutating the view list. Every successful proactive or reactive provider-view compaction persists a replacement snapshot when the store supports snapshots. With `compaction_ladder=None` the compaction/retry event sequence is byte-identical to the pre-ladder code (pinned by `test_ladder_disabled_is_byte_identical`). |
