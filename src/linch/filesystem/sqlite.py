"""Persistent SQLite-backed :class:`~linch.filesystem.backend.FileBackend`.

Use under :class:`~linch.filesystem.backend.CompositeFileBackend` to make a
subtree (e.g. ``/memories/``) survive across sessions.

All I/O runs on a single dedicated worker thread via
:class:`~linch.storage._executor.SqliteExecutor`, so the asyncio event loop
