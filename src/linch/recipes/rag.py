...
Exercises: :class:`~linch.context.ContextBuilder`, ``deps``,
:class:`~linch.types.OutputSchema`, tool-aware system prompt.

Quick start::

    from linch.recipes.rag import rag_agent, RagDeps
    ...
    **agent_kwargs: Any,
    ...
    Args:
        ...
        extra_instructions: Additional system instructions appended after the
            base RAG prompt.
        **agent_kwargs: Forwarded to :func:`~linch.recipes.build_agent`.

    Returns:
        An :class:`~linch.agent.Agent` with no SWE tools and per-turn
        document injection.
    """
    base_instructions = (
